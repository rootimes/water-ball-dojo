...
public class Individual {
  ...
  public Individual(
      int id, String gender, int age, String intro, String habitString, List<Float> coord) {
    if (!validateId(id)) throw new IllegalArgumentException("Invalid id: " + id);
    if (!validateGender(gender)) throw new IllegalArgumentException("Invalid gender");
    if (!validateAge(age)) throw new IllegalArgumentException("Invalid age: " + age);
    if (!validateIntro(intro)) throw new IllegalArgumentException("Invalid intro");
    if (!validateCoord(coord)) throw new IllegalArgumentException("Invalid coord");
  ...
  private boolean validateCoord(List<Float> coord) {
    return coord != null && coord.size() == 2 && coord.get(0) != null && coord.get(1) != null;
  }
