...
public class Main {
        public static void main(String[] args) {
                ...
                List<Individual> candidates = individuals.stream()
                                .filter(ind -> ind.getId() != target.getId())
                                .toList();
                ...
                List<Individual> matchedDistanceFirst = matcher.match(new DistanceBasedMatchStrategy(),
                                new TakeFirstPolicy());

                List<Individual> matchedDistanceLast = matcher.match(new DistanceBasedMatchStrategy(),
                                new TakeLastPolicy());

                List<Individual> matchedHabitFirst = matcher.match(new HabitBasedMatchStrategy(),
                                new TakeFirstPolicy());
                List<Individual> matchedHabitLast = matcher.match(new HabitBasedMatchStrategy(),
                                new TakeLastPolicy());
